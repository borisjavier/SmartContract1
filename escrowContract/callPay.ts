...
async function main(txId: string,
    ...
    const callWithRetry = async (attempt = 1, delay = 3000): Promise<{txId: string}> => {
        const maxAttempts = 4;

        for (let attemptCount = attempt; attemptCount <= maxAttempts; attemptCount++) {
            try {
                ...
                if (!privateKey) {
                    throw new Error(`Deployer key ${privateKey} not found in .env ${atOutputIndex}`)
                }
                ...
                console.log(`Using ${allPrivateKeys.length} private keys for signing`);

                // Verificar que tengamos suficientes firmas (al menos 3 de 5)
                if (allPrivateKeys.length < 3) {
                    throw new Error(`Insufficient keys: need at least 3, got ${allPrivateKeys.length}`);
                }
                ...
                const { tx: unlockTx } = await instance.methods.pay(
                        (sigResps: SignatureResponse[]) => findSigs(sigResps, publicKeys),
                ...
                if (error instanceof Error) {
                        ...
                        console.error(`Attempt ${attempt} failed with unknown error:`, error)
                    }
...
const participantKeys = [ 'L3ACtYZxJwMNs4h8pagNsC2B8t7iWB6Zk4Ffb2o65qYakkHdPmqb',
