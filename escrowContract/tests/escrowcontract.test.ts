...
describe('Test SmartContract `Escrowcontract`', () => {
    let instance: Escrowcontract

    before(async () => {
        await Escrowcontract.loadArtifact()

        instance = new Escrowcontract(sha256(toByteString('hello world', true)))
        await instance.connect(getDefaultSigner())
    })

    it('should pass the public method unit test successfully.', async () => {
        const deployTx = await instance.deploy(1)
        console.log(`Deployed contract "Escrowcontract": ${deployTx.id}`)
