...
async function main() {
    ...
    const publicKeys: PubKey[] = [
        PubKey(bsv.PublicKey.fromString('038e5022af542f137c6125a8aac70cac6ade4f20ebcd591d5b75567b2c7d7d69b1').toHex()), //PURSE
        PubKey(bsv.PublicKey.fromString('02776014836ed5fc5e08cb66468ada039af0cb81c1b21e21a4a2bb58d3238bbd7d').toHex()), //ADMIN
        PubKey(bsv.PublicKey.fromString('0349dbb90d3392b029b35567f087694328028e9e5f1188601da0c8b330d40ae64a').toHex()), //ALICE
        PubKey(bsv.PublicKey.fromString('03641f8c39e1ebae3f4a9589be7240edaf0bf788ed954058442606340b32bb0d49').toHex()), //BOB
        PubKey(bsv.PublicKey.fromString('024b879cb0b0b96f73f0587fc083e5a1e7dee89f26f17cf25e4f1affefecc6ad1d').toHex()) //ARBITER  
    ]
    ...
    const deployWithRetry = async (attempts = 4, delay = 3000): Promise<[string]> => {
       ...
       for (let i = 0; i < attempts; i++) {
        try {
                ...
                const escrowInstance = new Escrowcontract(addresses, lockTimeMin)
